...
router.post('/issue/report', requireAuth, (req, res) => {
  ...
  try {
    const proj = db.prepare('SELECT name FROM projects WHERE id = ?').get(Number(projectId));
    const recordId = Number(r.lastInsertRowid);
    if (proj && recordId) {
      const before = getQueueStats(proj.name);
      console.log(`[kbSync][issue] 入队前: projectName=${proj.name}, source=issue, recordId=${recordId}, action=upsert, queue=${JSON.stringify(before)}`);
      const eq = enqueueKbSync(proj.name, SYNC_SOURCE.ISSUE, recordId, 'upsert', 0);
      const after = getQueueStats(proj.name);
      console.log(`[kbSync][issue] 入队后: result=${JSON.stringify(eq)}, queue=${JSON.stringify(after)}`);
    } else {
      console.warn(`[kbSync][issue] 跳过入队: projectId=${projectId}, recordId=${recordId}`);
    }
  } catch (e) {
    console.error(`[kbSync][issue] 入队异常: ${e.message}`);
  }
