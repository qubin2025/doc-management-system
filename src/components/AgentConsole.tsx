      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">{projectName} — Agent自主规划执行</p>
          </div>
        </div>

        {/* 输入区 */}
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 mb-6">
          <label className="text-sm text-gray-400 mb-2 block">你想让AI做什么？</label>
          <div className="flex gap-3">
            <input
              ...
              disabled={running}
              className="flex-1 bg-gray-800 border border-gray-700 rounded-lg px-4 py-2.5 text-sm focus:border-purple-500 focus:outline-none disabled:opacity-50"
            />
            {running ? (
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 mb-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              {engineeringAgent.listTools().map(t => (
                <div key={t.name} className="px-3 py-2 bg-gray-800 rounded-lg text-xs">
                  ...
                  <div className="text-gray-600 mt-0.5 truncate">{t.description}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        {task && (
          <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
            <div className="px-4 py-3 border-b border-gray-800 flex items-center justify-between">
              ...
              <span className="text-xs text-gray-500">{task.steps.filter(s => s.status === 'completed').length}/{task.steps.length} 步</span>
            </div>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="text-xs text-gray-500">步骤 {i + 1}</span>
                        {step.actionName && (
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-gray-900 border border-gray-700 rounded-xl p-6 max-w-sm w-full mx-4">
            ...
            {Object.keys(confirmAction.params).length > 0 && (
              <pre className="text-xs text-gray-500 bg-gray-800 p-2 rounded mt-2 mb-3 max-h-32 overflow-y-auto">
