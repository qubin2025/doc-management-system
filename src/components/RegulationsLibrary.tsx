        ...
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50" onClick={() => setEditDoc(null)}>
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg max-h-[85vh] overflow-y-auto p-6" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3"><div><label className="block text-xs text-gray-500 mb-1">分类</label><select value={editDoc.category} onChange={e => setEditDoc(p => ({ ...p!, category: e.target.value }))} className="w-full px-3 py-2 border rounded-lg text-sm">{CATEGORIES.filter(c => c !== '全部').map(c => <option key={c} value={c}>{c}</option>)}</select></div><div><label className="block text-xs text-gray-500 mb-1">文号</label><input value={editDoc.fileNo} onChange={e => setEditDoc(p => ({ ...p!, fileNo: e.target.value }))} className="w-full px-3 py-2 border rounded-lg text-sm" /></div><div><label className="block text-xs text-gray-500 mb-1">颁布单位</label><input value={editDoc.department} onChange={e => setEditDoc(p => ({ ...p!, department: e.target.value }))} className="w-full px-3 py-2 border rounded-lg text-sm" /></div></div>
              <div className="grid grid-cols-2 gap-3"><div><label className="block text-xs text-gray-500 mb-1">生效时间</label><input value={editDoc.publishDate} onChange={e => setEditDoc(p => ({ ...p!, publishDate: e.target.value }))} className="w-full px-3 py-2 border rounded-lg text-sm" /></div><div><label className="block text-xs text-gray-500 mb-1">状态</label><select value={editDoc.status} onChange={e => setEditDoc(p => ({ ...p!, status: e.target.value as any }))} className="w-full px-3 py-2 border rounded-lg text-sm"><option value="现行有效">现行有效</option><option value="已废止">已废止</option><option value="即将实施">即将实施</option></select></div></div>
            ...
            <div className="flex justify-end gap-2 mt-4 pt-4 border-t"><button onClick={() => setEditDoc(null)} className="px-4 py-2 border rounded-lg text-sm">取消</button><button onClick={saveEdit} className="px-4 py-2 bg-blue-500 text-white rounded-lg text-sm flex items-center gap-1"><Save className="w-4 h-4" />保存</button></div>
          </div>
        </div>
      )}
    </div>
