      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 dark:border-slate-700">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-lg font-bold text-slate-800 dark:text-slate-200">AI 解析提示词配置</h2>
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => setShowRaw(!showRaw)}
              className="px-3 py-1.5 text-xs border border-slate-300 dark:border-slate-600 rounded-lg text-slate-500 dark:text-slate-400 flex items-center gap-1">
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {showRaw ? (
            ...
            <div className="space-y-3">
              <p className="text-xs text-slate-400 dark:text-slate-500">以下为发送给 DeepSeek 的完整提示词（只读）</p>
              <pre className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-4 text-xs text-slate-600 dark:text-slate-400 whitespace-pre-wrap leading-relaxed max-h-[60vh] overflow-y-auto">
{`${rules.map(r => `【${r.element}】${r.practice}`).join('\n\n')}\n${extraRules ? '\n' + extraRules + '\n' : ''}\n请严格按照以上规则提取JSON，只返回JSON不要其他内容。`}
            ...
            <div>
              <table className="w-full text-sm border-collapse">
                <thead>
                  <tr className="bg-slate-100 dark:bg-slate-700">
                    <th className="text-left px-3 py-2 text-xs font-semibold text-slate-500 dark:text-slate-400 w-[12%]">要素</th>
                    <th className="text-left px-3 py-2 text-xs font-semibold text-slate-500 dark:text-slate-400 w-[18%]">说明</th>
                    <th className="text-left px-3 py-2 text-xs font-semibold text-slate-500 dark:text-slate-400">本项目实践（可编辑）</th>
              ...
              {/* Extra prompt text */}
              <div className="mt-4">
                <label className="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">附加提示词（追加在规则末尾）</label>
                <textarea
                  value={extraRules}
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/50 rounded-b-2xl">
          <div className="flex items-center gap-2">
            <button onClick={handleReset}
              className="flex items-center gap-1.5 px-4 py-2 text-sm border border-slate-300 dark:border-slate-600 rounded-xl text-slate-500 dark:text-slate-400 hover:bg-slate-100 transition-colors">
