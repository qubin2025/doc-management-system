      ...
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">{projectName} — {skills.length} 个可用技能</p>
          </div>
        </div>
          ...
          {categories.map(c => (
            <button key={c.id} onClick={() => setFilterCategory(c.id)}
              className={`px-3 py-1.5 text-sm rounded-lg transition ${
                filterCategory === c.id ? 'bg-amber-500/20 text-amber-400 border border-amber-500/30' : 'bg-gray-800 text-gray-400 hover:bg-gray-700'
              }`}>
              {c.label}
            </button>
          ))}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {skills.map(s => (
              <button key={s.id} onClick={() => setActiveSkill(s.id)}
                className="bg-gray-900 border border-gray-800 rounded-xl p-5 text-left hover:border-amber-500/30 transition group">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <span className="inline-block mt-2 px-1.5 py-0.5 text-[10px] bg-gray-800 rounded text-gray-400">{s.category}</span>
                  </div>
                </div>
              </button>
            ))}
            ...
            {result && (
              <div className={`bg-gray-900 border rounded-xl p-5 ${result.success ? 'border-green-500/30' : 'border-red-500/30'}`}>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-xs text-gray-500 ml-2">{(result.duration / 1000).toFixed(1)}s</span>
                </div>
                {result.report && <p className="text-sm text-gray-400 mb-2">{result.report}</p>}
                {result.data != null && (
                  <div className="bg-gray-800 rounded-lg p-3 text-xs text-gray-300 max-h-48 overflow-y-auto whitespace-pre-wrap">
